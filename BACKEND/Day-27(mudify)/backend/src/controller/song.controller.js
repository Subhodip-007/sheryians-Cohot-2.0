...
const songUploadController = async (req, res) => {
    try {
        ...
        // Run both file uploads concurrently
        const [songFile, posterFiles] = await Promise.all([
            storageService.uplaodFile({
                buffer: songBuffer,
                fileName: `${tags.title}.mp3`,
                folder: "/cohot-2/mudify/songs"
            }),
...
const getSongController =async (req,res)=>{
    const {mood} = req.query //  query looks like http://localhost:3000/api/songs/get?mood = sad
